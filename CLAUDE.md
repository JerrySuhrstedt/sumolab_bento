...
- **Redirects:** hand-written rules live in `public/_redirects`. `scripts/trailing-slash-redirects.mjs` runs as `postbuild` and prepends one 308 rule per prerendered page so `/path` answers 308, not Cloudflare's default 307, plus a 301 from the old WordPress root URL (`/slug/` and `/slug`) to `/blog/slug/` for every article, since the old site served articles at the root. Keep static rules above the `/insights/*` splat rule: rules placed after it stop being applied past roughly the 113th. Supported status codes are 301, 302, 303, 307, 308 (no 410).
...
- **Auth, two layers:** (1) Cloudflare Access application "SumoLab Editorial" on `sumolab.co/admin` (Zero Trust org `throbbing-wave-ac21.cloudflareaccess.com`, app id `237715ca-e8b8-4008-80bd-1ebabb9bd965`, email one-time code, allow list jerry@sumolab.co and jerrysuhrstedt@gmail.com, 24h session). Bots never reach the Worker. Add a client later by adding their email to the policy. (2) `src/middleware.ts` gates `/admin` with an HMAC cookie derived from the `ADMIN_PASSWORD` Worker secret (`src/lib/admin-auth.ts`). Local passphrase is in `.dev.vars`; production passphrase is in the vault at `seo-content/README.md`. Rotate with `npx wrangler secret put ADMIN_PASSWORD`. Local `wrangler dev` has no Access layer, so curl tests hit the passphrase directly.
...
- **Why this topic panel:** every article card carries a snapshot of the keyword numbers as they stood when the idea was proposed (`snapshot_volume`, `snapshot_competition`, `snapshot_score`, `snapshot_taken_at`, `snapshot_source` on `editorial_articles`). The article page shows the source question, volume, competition, score, target page, and data age. Past 90 days the age turns orange and warns to re-import; past 180 it turns red. If the bank's current numbers differ from the snapshot, the panel says so. Board cards show volume, competition, and a colored dot when the data is aging.
- **Writing prompt:** every outline and draft follows the vault file `seo-content/article-prompt_09-05-2026.md` (Phase 1 outline, Phase 2 draft). Read it before writing.
